...
export class AdminNodeFeesComponent implements OnInit {
  ...
  ngOnInit() {
    this.backendApi
      .AdminGetTransactionFeeMap(this.globalVars.localNode, this.globalVars.loggedInUser?.PublicKeyBase58Check)
      .subscribe(
        (res) => {
  ...
  addNewFee(txnType: string) {
    ...
    addFeeModal.content.feeAdded.subscribe((transactionFeeMap) => (this.transactionFeeMap = transactionFeeMap));
  }

  removeFee(txnType: string, publicKeyBase58Check: string): void {
    let transactionFeesForType = this.transactionFeeMap[txnType];
    const txnFeeIndex = transactionFeesForType.findIndex((txn) => txn.PublicKeyBase58Check === publicKeyBase58Check);
    if (txnFeeIndex < 0) {
      return;
    }
    if (this.removingFee) {
    ...
    SwalHelper.fire({
      ...
      html: `Remove fee of ${this.globalVars.nanosToDeSo(txnFee.AmountNanos)} DESO for ${
