locals {
  ...
  replica_count     = 1
  docker_repository = "093482933631.dkr.ecr.us-east-1.amazonaws.com/frontend"
  docker_tag        = "v2.1.1.0-testnet"
  requests_memory   = "1Gi"
  ...
  tags = {
    ...
    ops_source_repo_path = "terraform-environments/aws/testnet",
...
terraform {
  ...
  backend "remote" {
    ...
    workspaces {
      name = "kubernetes-ops-testnet-70-gem-frontend"
    }
  }
}
...
data "terraform_remote_state" "eks" {
  ...
  config = {
    organization = "gem-engineering"
    workspaces = {
      name = "kubernetes-ops-testnet-20-eks"
    }
  }
}
